...
Value getstakinginfo(const Array& params, bool fHelp)
{
	...
	if (pindexBest->nHeight > POS_JUMPSTART_FORK_BLOCK+1){
		nExpectedTime = staking ? (GetTargetSpacingV2(nBestHeight) * nNetworkWeight / nWeight) : 0;
	}else{
		nExpectedTime = staking ? (GetTargetSpacingV1(nBestHeight) * nNetworkWeight / nWeight) : 0;
	}	
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BDRCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BDRCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BDRCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BDRCoin is downloading blocks...");
