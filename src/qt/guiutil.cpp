...
{
    // Convert bdrcoin:// to bdrcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bdrcoin://"))
    {
        uri.replace(0, 12, "bdrcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BDRCoin.lnk";
}
...
{
    return GetAutostartDir() / "bdrcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BDRCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bdrcoin-qt [" + tr("command-line options") + "]                     " + "\n";
