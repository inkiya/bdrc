...
#if defined(NDEBUG)
# error "BDRCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BDRCoin Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
	...
	if (pindexBest->nHeight > POS_JUMPSTART_FORK_BLOCK+1){
		return max(0, nCoinbaseMaturity - GetDepthInMainChain()); //120 Confirmations
	}else{
		return max(0, (nCoinbaseMaturity - 70) - GetDepthInMainChain());	//50 Confirmations
	}	
}
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 800000 * COIN;   //800K Premine 
    }
        else
    {
        nSubsidy = 0 * COIN;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (pindexBest->nHeight > POS_JUMPSTART_FORK_BLOCK+1){
		nTargetSpacing = GetTargetSpacingV2(pindexLast->nHeight);
	}else{
		nTargetSpacing = GetTargetSpacingV1(pindexLast->nHeight);
	}	
    ...
    if (IsProtocolV3(pindexLast->nHeight)) {
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (IsProtocolV3(pindexBlock->nHeight))
            {
        ...
        if (!IsCoinStake())
        {
            ...
            if (!IsProtocolV3(pindexBlock->nHeight+1)) {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProtocolV3(pindexBest->nHeight))
    {
...
bool CBlock::CheckBlockSignature() const
{
	...
	else if (IsProtocolV3(pindexBest->nHeight))	
    {
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (TestNet())
    {
        ...
        nCoinbaseMaturity = 60; // test maturity is 60 blocks
    }
